import { _ as resolveConfigDir, b as resolveUserPath, c as ensureDir, l as escapeRegExp, t as CONFIG_DIR } from "./utils-BTaR--Ln.js";
import { Nt as resolveNodeManagerOptions, Or as loadModelCatalog, Ot as openUrl, bt as detectBinary } from "./reply-Cb7UHA2y.js";
import { t as runCommandWithTimeout } from "./exec-DFOtZbI0.js";
import { a as resolveAgentModelPrimary, c as resolveDefaultAgentId, r as resolveAgentDir, s as resolveAgentWorkspaceDir, w as resolveDefaultAgentWorkspaceDir } from "./agent-scope-DnyDZ5RH.js";
...
import { n as resolveWideAreaDiscoveryDomain } from "./widearea-dns-BgYasW6m.js";
...
async function promptCustomApiConfig(params) {
	...
	let providerApi = COMPATIBILITY_OPTIONS.find((entry) => entry.value === compatibility)?.api ?? "openai-completions";
	while (true) {
	...
	if (providerIdResult.renamed) await prompter.note(`Endpoint ID "${providerIdInput}" already exists for a different base URL. Using "${providerIdResult.providerId}".`, "Endpoint ID");
	const providerId = providerIdResult.providerId;
	const modelRef = modelKey(providerId, modelId);
	const alias = (await prompter.text({
		message: "Model alias (optional)",
		placeholder: "e.g. local, ollama",
		initialValue: "",
		validate: (value) => resolveAliasError({
			raw: value,
			cfg: config,
			modelRef
		})
	...
	const normalizedApiKey = apiKey.trim() || (existingApiKey ? existingApiKey.trim() : void 0);
	let newConfig = {
		...config,
		models: {
			...config.models,
...
//#endregion
export { promptAuthChoiceGrouped as _, promptCustomApiConfig as a, applyPrimaryModel as c, resolvePreferredProviderForAuthChoice as d, warnIfModelConfigLooksOff as f, applyGoogleGeminiModelDefault as g, upsertSharedEnvVar as h, discoverGatewayBeacons as i, promptDefaultModel as l, applyOpenAIConfig as m, installSkill as n, applyModelAllowlist as o, applyAuthChoice as p, promptRemoteGatewayConfig as r, applyModelFallbacksFromSelection as s, setupSkills as t, promptModelAllowlist as u };